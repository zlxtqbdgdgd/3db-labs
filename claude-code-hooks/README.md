...
该目录是可单独分发的 client kit；用户不需要取得或克隆 3db-mcp 仓库。

> **默认没有——必须安装**（见下文《安装》，5 步，每步有命令）。
> **装好后怎么触发**：给 Claude Code 发一条以 `诊断:`（或 `diag:`，冒号全半角都认）开头的消息
> ——一条消息 = 一棵 trace 树；普通消息零足迹。全量记录用 `TDB_OBS_MODE=always`。

设计总纲：3db-web `docs/design/topic-agent-observability.md`；mcp 侧契约：`docs/adr/0008`。
...
| 2 | 3db-mcp server 已在 Claude Code 配置 | 否 | `claude mcp add --transport http 3db-mcp http://<mcp地址>/mcp` | `claude mcp list` |
...
| 5 | 上报通道（llm/root span 入库用） | 否（不配=只落本地） | 《安装》Step 4 | `curl -s -o /dev/null -w '%{http_code}' -X POST http://<mcp地址>/api/v2/llmobs/spans -H "DD-API-KEY: $TDB_OBS_API_KEY" -d '{"spans":[]}'` |
...
| `pre-tool-use.mjs` | **传播**：matcher `mcp__3db.*`，出站前经 `updatedInput` 把 `telemetry.trace_id/parent_span_id` 盖上（intent 仍由模型填）；不设 permissionDecision，权限流照常 |
...
**Step 1 — 取 kit，定路径**（本目录随公开仓 `zlxtqbdgdgd/3db-labs` 分发：clone 本仓、或从
3db 控制台 `/downloads/client-kit/claude-code-hooks.tar.gz` 下载解包；更省事的插件安装
...
```sh
sed "s|/ABSOLUTE/PATH/TO/3db-client-kit/claude-code-hooks|$KIT|g" "$KIT/settings-snippet.json"
```
...
cp "$S" "$S.bak"
jq --slurpfile snip <(sed "s|/ABSOLUTE/PATH/TO/3db-client-kit/claude-code-hooks|$KIT|g" "$KIT/settings-snippet.json") \
   '.hooks = ((.hooks // {}) + $snip[0].hooks)' "$S" > "$S.new" && mv "$S.new" "$S"
jq -e '.hooks | keys' "$S"   # 应含 UserPromptSubmit / PreToolUse / Stop / SubagentStop
```

**Step 4 — 配上报通道**（不配也能跑：span 只落本地 JSONL）。3db 控制台 **settings → api-keys**
签发 `3db_` 前缀 key，然后把 `Stop`/`SubagentStop` 两处命令里的占位换掉：

```sh
sed -i.bak \
  -e "s|http://<3db-mcp地址>/api/v2/llmobs/spans|http://<实际mcp地址>/api/v2/llmobs/spans|g" \
  -e "s|<填入3db_开头的API key>|3db_xxxxxxxx|g" ~/.claude/settings.json
```
...
export TDB_OBS_DIR=$(mktemp -d)
echo '{"session_id":"selftest","prompt":"诊断：自检","cwd":"'$PWD'","transcript_path":"/dev/null"}' \
  | node "$KIT/user-prompt-submit.mjs" && cat "$TDB_OBS_DIR/selftest.json"
# 预期：{"active":true,"trace_id":"<32hex>","root_span_id":"<前16hex>",...}

# 2) 注入点：telemetry 应被盖上同一 trace 上下文
echo '{"session_id":"selftest","tool_name":"mcp__3db-mcp__list_llmobs_projects","tool_input":{"telemetry":{"intent":"t"}}}' \
...
ls ~/.claude/3db-obs/                 # 应出现 <session_id>.json
tail -3 ~/.claude/3db-obs/spans.jsonl # 应有 kind:"agent"(root) 与 kind:"llm" 的行
# LLM Obs UI 按 ml_app:<目录名> 过滤 traces，应看到完整树（root → llm/tool span）
```

## 产物与配置

- 状态：`~/.claude/3db-obs/<session_id>.json`（trace 上下文 + transcript 游标）
- span：`~/.claude/3db-obs/spans.jsonl`（每行一个 span，本地真相源永远先落）
- **上报 3db**（Phase C，ADR-0034）：设 `TDB_OBS_REPORT_URL`（`http://<3db-mcp 地址>/api/v2/llmobs/spans`）
  + `TDB_OBS_API_KEY`（控制台 settings/api-keys 签发）后，Stop 合成的 span 会 best-effort
  从 MCP 边缘口 POST 入库；未设两 env = 只落本地，行为与 Phase A 相同
- env：`TDB_OBS_DIR`（状态/产物目录）、`TDB_OBS_SPANS`（spans 路径）、
  `TDB_OBS_CONTENT_CHARS`（内容截断，默认 8000，对齐 `TDB_TELEMETRY_OUTPUT_CHARS` 先例）、
  `TDB_OBS_ML_APP`（应用名标签，打进 root/llm span 的 `tags.ml_app`；缺省 = 项目目录名。
  复盘按它过滤——同一台机器上编码会话与真诊断靠它分开）

## 触发门（TDB_OBS_MODE，2026-07-11）
...
| 模式 | 行为 | 用在哪 |
|------|------|--------|
| `triggered`（默认） | prompt 以触发词开头才记（`TDB_OBS_TRIGGER`，默认 `诊断:`；`diag:` 恒收；**冒号全半角都认**——中文输入法的「：」照样触发） | 日常工作仓（如 3db-web） |
| `always` | 每条消息都记 | 诊断专用目录（如 3db-test，配在 UserPromptSubmit 命令前） |
| `off` | 彻底关闭 | — |
...
| 症状 | 最可能原因 | 处置 |
|------|-----------|------|
| 一个 span 都没有 | 消息没带触发词 | 以 `诊断:` 开头重发；或 `TDB_OBS_MODE=always` |
...
| tool span 有，llm/root span 只在本地 `spans.jsonl` | 上报 env 没配齐（两个都要） | 补 `TDB_OBS_REPORT_URL` + `TDB_OBS_API_KEY`（安装 Step 4），用前提清单第 5 行的 curl 验证 |
